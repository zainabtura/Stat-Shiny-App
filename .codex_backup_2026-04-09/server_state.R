...
register_core_server <- function(input, output, session, state) {
  ...
  observe({
    ...
    enable_targets(c("nav_regression", "nav_adequacy", "card_adequacy", "nav_corrections", "card_corrections"), length(nums) >= 2)
    ...
    allowed_tabs <- c(
      ...
      if (length(nums) >= 2) c("tab_regression", "tab_adequacy", "tab_corrections")
    )
...
register_preview_server <- function(input, output, session, state) {
          ...
          if (is.null(input$file)) {
            ...
            div(
              ...
              div(
                ...
                withSpinner(DTOutput("upload_preview_table", width = "100%"), color = "#0f766e", type = 4)
              )
            )
          }
        )
  ...
  output$descriptive_scatter <- renderPlot({
    ...
    plot(
      ...
      pch = 19,
      col = adjustcolor("#0f766e", alpha.f = 0.72),
      cex = 1.4,
      cex.main = 1.5,
      cex.lab = 1.2,
      col.main = "#132a23",
  ...
  output$descriptive_stacked <- renderPlot({
    ...
    cols <- c("#0f766e", "#1d4ed8", "#ec4899", "#10b981", "#f97316")
  ...
  output$descriptive_mosaic <- renderPlot({
    req(state$data_in(), input$dataset_var1, input$dataset_var2)
    tab <- table(state$data_in()[[input$dataset_var1]], state$data_in()[[input$dataset_var2]], useNA = "ifany")
    mosaicplot(tab, main = "Mosaic Plot", color = c("#0f766e", "#1d4ed8", "#ec4899", "#10b981"), cex.axis = 1, col.main = "#132a23", cex.main = 1.3)
  })
    ...
    switch(
        ...
        if (is.numeric(x)) {
          div(
            ...
            div(
              ...
              div(
                class = "plot-row",
                withSpinner(plotOutput("descriptive_hist", height = "400px"), color = "#0f766e", type = 4),
                withSpinner(plotOutput("descriptive_box", height = "400px"), color = "#0f766e", type = 4)
              )
            )
          )
          ...
          div(
            ...
            div(
              ...
              div(
                class = "plot-row",
                div(class = "table-card", h4("Frequency Table"), withSpinner(DTOutput("descriptive_single_freq_table"), color = "#0f766e", type = 4)),
                div(class = "table-card", h4("Proportion Table"), withSpinner(DTOutput("descriptive_single_prop_table"), color = "#0f766e", type = 4))
              )
            ),
            div(
              class = "content-card",
              ...
              div(
                class = "plot-row",
                withSpinner(plotOutput("descriptive_bar", height = "420px"), color = "#0f766e", type = 4),
                withSpinner(plotOutput("descriptive_pie", height = "420px"), color = "#0f766e", type = 4)
              )
            )
          )
        } else {
        ...
        if (is.numeric(x) && is.numeric(y)) {
          div(
            ...
            div(
              ...
              withSpinner(plotOutput("descriptive_scatter", height = "520px"), color = "#0f766e", type = 4)
            )
          )
        } else if (is_categorical(x) && is_categorical(y)) {
          div(
            ...
            div(
              ...
              div(class = "table-card", h4("Frequency Table"), withSpinner(DTOutput("descriptive_freq_table"), color = "#0f766e", type = 4)),
              div(class = "table-card", h4("Proportions"), withSpinner(DTOutput("descriptive_prop_table"), color = "#0f766e", type = 4))
            ),
            div(
              class = "content-card",
              div(class = "card-header", "Comparative Plots"),
              fluidRow(
                column(4, withSpinner(plotOutput("descriptive_stacked", height = "420px"), color = "#0f766e", type = 4)),
                column(4, withSpinner(plotOutput("descriptive_side", height = "420px"), color = "#0f766e", type = 4)),
                column(4, withSpinner(plotOutput("descriptive_mosaic", height = "420px"), color = "#0f766e", type = 4))
              )
            )
          )
        } else {
      ...
      "Three Variables Analysis" = div(
        ...
        div(
          ...
          withSpinner(DTOutput("dataset_corr_tbl"), color = "#0f766e", type = 4)
        ),
        div(
          class = "content-card",
          ...
          div(
            class = "plot-row",
            withSpinner(plotOutput("dataset_pairs_plot", height = "500px"), color = "#0f766e", type = 4),
            withSpinner(plotOutput("dataset_corr_plot", height = "500px"), color = "#0f766e", type = 4)
          )
        )
      )
    )
  ...
  output$dataset_pairs_plot <- renderPlot({
    ...
    vars <- c(input$dataset_multi_var1, input$dataset_multi_var2, input$dataset_multi_var3)
    nums <- state$data_in()[, vars, drop = FALSE]
    pairs(nums, main = "Selected Numeric Variables", pch = 19, col = adjustcolor("#0f766e", alpha.f = 0.55), cex = 0.8, col.main = "#132a23", cex.main = 1.4)
  })
  ...
  output$dataset_corr_plot <- renderPlot({
    ...
    nums <- state$data_in()[, vars, drop = FALSE]
    cm <- cor(nums, use = "complete.obs")
    corrplot(cm, method = "circle", type = "full", diag = FALSE, col = colorRampPalette(c("#ef4444", "white", "#0f766e"))(100), tl.col = "#132a23", tl.cex = 1)
  })
}
